...
const Todo = ({ avatar_url, login }) => {
    ...
    const add = () => {
        console.log("Add");
        if (name == "")
            return alert("กรุณากรอกข้อมูลในช่องว่าง");
        if (tasks.length >= 10)
            return alert("ไม่สามารถเพิ่มข้อมูลเกิน 10 ครั้ง");
        if (tasks.length == 0)
            setTasks([{ id: 1, name }]);
        else
    ...
    return (
        <div className="bg-indigo-400 h-screen flex flex-col items-center p-8" >
